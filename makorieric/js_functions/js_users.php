<?php
	function js_signin() {
		...
		$loginkey = md5($_POST['password']);
			...
			$_SESSION['nyakeminchasecondary_user'] = js_let_me_user($loginname, $loginkey);
			$_SESSION['nyakeminchasecondary_account'] = js_logged_account($loginname);
	...
	function js_is_logged(){
		$myloginid = isset( $_SESSION['nyakeminchasecondary_user'] ) ? $_SESSION['nyakeminchasecondary_user'] : "";
	...
	function js_signin_modal() {
	  if ( isset( $_POST['LetMeIn'] ) ) {
		...
		if( $database->js_num_rows( $js_db_query ) > 0 ) {
		    ...
		    $_SESSION['nyakeminchasecondary_user'] = $userid;			
...
function logout() {
  unset( $_SESSION['nyakeminchasecondary_user'] );
  unset( $_SESSION['nyakeminchasecondary_account'] );
  header( "Location: index.php" );
}
	...
	function js_add_new_topic(){
 		$raw_file_name = basename($_FILES["avatar"]["name"]);
		$temp_file_name = $_FILES["avatar"]["tmp_name"];		
		$upload_file_ext = explode(".", $raw_file_name);
		$upload_file_name = preg_replace("/-$/","",preg_replace('/[^a-z0-9]+/i', "_", strtolower($upload_file_ext[0])));
		$finalname = 'topic'.time().'.'.$upload_file_ext[1];
		$target_file = JS_TARGET . $finalname;
		$imageFileClass = pathinfo($target_file,PATHINFO_EXTENSION);		
		if (move_uploaded_file($temp_file_name, $target_file)) $js_avatar = $finalname;
		else $js_avatar = "topic_default.jpg";		
			 
		$database = new Js_Dbconn();			
		$New_User_Details = array(			
			'topic_name' => trim($_POST['topicname']),
			'topic_fullname' => trim($_POST['fullname']),
			'topic_email' => trim($_POST['email']),
			'topic_mobile' => trim($_POST['mobile']),
			'topic_address' => trim($_POST['address']),
			'topic_avatar' => trim($js_avatar),
			'topic_joined' => date('Y-m-d H:i:s'),
		);
			
		$add_query = $database->js_insert( 'js_topic', $New_User_Details ); 
	}
