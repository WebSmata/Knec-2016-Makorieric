<?php
	...
	function my_item_cart($cartno) {
		$my_db_query = "SELECT * FROM my_class WHERE catid = '$cartno'";
	...
	{
		echo '<div class="product menu-class">
				<a href="'.my_siteLynk().$row['item_slug'].'"><div class="menu-class-name list-group-item active">'.my_item_cart($row['item_cat']).'</div></a>
				<a href="'.my_siteLynk().$row['item_slug'].'"><div class="product-image">
					<img class="product-image menu-item list-group-item" src="'.my_siteLynk_img().$row['item_img'].'" />
	...
	function my_lookup_cat($request){
		$my_db_query = "SELECT * FROM my_class WHERE cat_slug = '$request'";
		$database = new Js_Dbconn();
