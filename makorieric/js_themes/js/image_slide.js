...
function slideShow(speed) {
  ...
  $('ul.slideshow li:first').css({opacity: 1.0}).addItem('show');
...
function gallery() {
  ...
  if(current.queue('fx').length == 0) {
    ...
    next.css({opacity: 0.0}).addItem('show').animate({opacity: 1.0}, 1000);
    ...
    current.animate({opacity: 0.0}, 1000).removeItem('show');
